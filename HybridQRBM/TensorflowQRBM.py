"""Tensorflow Dynex Neuromporhic Layer."""
...
class QRBM(tf.keras.layers.Layer):
    def __init__(self, num_hidden=1, steps_per_epoch=1,
                 sampler="Sampler", 
        ...
        if name is None:
            name = "QRBM_layer_" + "".join(self.rnd.choice(list(letters), size=10))
        self.name_ = name
        ...
        # TF buffers:
        self.model_nodes = tf.Variable(initial_value=tf.constant(0), trainable=False)
        self.model_weights = tf.Variable(initial_value=tf.constant(0), trainable=False)
        self.model_biases_visible = tf.Variable(initial_value=tf.constant(0), trainable=False)
        self.model_biases_hidden = tf.Variable(initial_value=tf.constant(0), trainable=False)
    
    #def build(self, input_shape):
        # Create weights of the layer
    ...
    def to_qubo_matrix(self, data: np.ndarray = None, clamp_strength: float = 2.0):
        ...
        blocks = [
            [np.zeros((self.num_visible, self.num_visible)), self.weights_],
    ...
    def _fit_batch(self, data: npt.NDArray) -> float:
        ...
        self.weights_ += delta.weights
        self.biases_visible += delta.biases_visible
        self.biases_hidden += delta.biases_hidden

        # Apply sample on batch:
        print('DynexQRBM tensorflow Layer | applying sampling result...', len(self.data),'x',len(self.data[0]));
    ...
    def call(self, x):
        ...
        if self.weights_ is None:
            self.weights_ = self.rnd.normal(scale=0.001, size=(self.num_visible, self.num_hidden));
        ...
        self.logger.info(f"DynexQRBM Tensorflow Layer | batch data appended:  {self.cnt}")
        print('DynexQRBM Tensorflow Layer | batch data appended:',self.cnt);
            
        # end of batch?
        if self.cnt % self.steps_per_epoch == 0:
            print('DynexQRBM Tensorflow Layer | end of batch, sampling...', len(self.data),'x',len(self.data[0]));
            self.data = np.array(self.data);
            error += self._fit_batch(self.data);
            error /= self.steps_per_epoch; 
            self.errors.append(error);
            self.logger.info(f"DynexQRBM Tensorflow Layer | SME:  {error}")
            acc = ((1-error)*100);
            self.acc.append(acc);
            self.logger.info(f"DynexQRBM Tensorflow Layer | ACCURACY: {acc}%")
            print('DynexQRBM Tensorflow Layer | SME:','{:f}'.format(error),'ACCURACY:','{:f}%'.format(acc));
            # Update Tensorflow buffers:
            self.model_dmodel_weight = tf.convert_to_tensor(self.weights_);
            self.model_biases_visible = tf.convert_to_tensor(self.biases_visible);
            self.model_biases_hidden = tf.convert_to_tensor(self.biases_hidden);
            # reset data container
            self.data = []; 
            
        return tf.convert_to_tensor(self.model_biases_hidden);   # hidden nodes are returned for transfer learning
    ...
    def save_model(self, file):
        ...
        """
        checkpoint = {'weights':self.weights_,'biases_visible':self.biases_visible,'biases_hidden':self.biases_hidden}
    ...
    def load_model(self, file):
        ...
        self.biases_visible = checkpoint['biases_visible'] 
        self.biases_hidden = checkpoint['biases_hidden']        

# --------------------------------------------------------------------------------------------------------------------------------------------------------
# QRBM Experimental class:
# --------------------------------------------------------------------------------------------------------------------------------------------------------
class dnx_experimental(tf.keras.layers.Layer):
    """ Dynex QRBM Layer """
    ...
    def sample_opposite_layer_pyqubo(self, v, layer, weights, opposite_layer):
        ...
        dnxsampler = dynex.DynexSampler(dnxmodel, logging=self.logging, mainnet=self.mainnet, description='Tensorflow QRBM');
    ...
    def call(self, x):
        ...
        return tf.convert_to_tensor(sample_output)